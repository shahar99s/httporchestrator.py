...
from fetchers.utils import Mode, format_size, should_download
...
class SmashFetcherFactory:
    ...
    def create(self_factory, mode: Mode = Mode.FETCH, **kwargs) -> BaseFetcher:
        class SmashFetcher(BaseFetcher):
            ...
            def extract_filename(self, files_metadata: dict, transfer_metadata: dict) -> str:
                ...
                # Multiple files: use transfer name as zip filename
                name = transfer_metadata.get("transfer_name") or self_factory.transfer_id
                return f"{name}.zip"
            ...
            def __init__(self, **kwargs):
                ...
                info_steps = [
                    ...
                    .get(
                        lambda v: f"https://transfer.{v['transfer_region']}.fromsmash.co/transfer/{v['public_transfer_id']}/preview"
                    )
                    .headers(**self_factory.headers, Authorization="Bearer $account_token")
                    ...
                    .get(
                        lambda v: f"https://transfer.{v['transfer_region']}.fromsmash.co/transfer/{v['public_transfer_id']}/files/preview"
                    )
                    .headers(**self_factory.headers, Authorization="Bearer $account_token")
                    .params(version="01-2024", e=self_factory.identity_token)
                    .teardown_callback("extract_files_metadata(response, target)", assign="files_metadata")
                    .teardown_callback("extract_filename(files_metadata, transfer_metadata)", assign="filename")
