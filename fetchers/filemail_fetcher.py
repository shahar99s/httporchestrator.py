                    ...
                    "fetch snapshot",
                    {
                        "summary": {
                            ...
                            "upload_date": format_timestamp(
                                transfer.get("created") if isinstance(transfer, dict) else None
                            ),
                            "expires_at": format_timestamp(
                                transfer.get("expires") if isinstance(transfer, dict) else None
                            ),
                ...
                [
                    ConditionalStep(
                        ...
                        lambda vars: should_download(
                            mode,
                            # No sender email means no notification risk — safe to download
                            vars.get("downloads_count") or (1 if not vars.get("metadata", {}).get("from") else 0),
                        )
                    )
                ]
            )
