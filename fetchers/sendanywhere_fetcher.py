...
from fetchers.utils import (Mode, format_timestamp, resolve_filename,
                            should_download)
...
class SendAnywhereFetcherFactory:
    ...
    def __init__(self, link: str, headers: Dict[str, str] | None = None):
        ...
        if host == "mandrillapp.com" and "sendanywhe.re" in link:
    ...
    def create(self, mode: Mode = Mode.FETCH, **kwargs) -> BaseFetcher:
        ...
        class SendAnyWhereFetcher(BaseFetcher):
            ...
            def extract_filename_from_header(self, response: ResponseObject) -> str:
            ...
            def update_metadata_from_search(self, metadata: dict, response: ResponseObject) -> dict:
                """Enrich metadata with file_count/total_size from /web/key/search."""
                data = response.json or {}
                metadata["file_count"] = data.get("file_count", metadata.get("file_count"))
                metadata["total_size"] = data.get("file_size", metadata.get("total_size"))
                return metadata
            ...
            fetch_steps.extend(
                [
                    # Relay keys: search endpoint has file_count/file_size/weblink
                    ConditionalStep(
                        RunRequest("get relay file info")
                        .post(f"/web/key/search/{key}")
                        .json({})
                        .teardown_callback("update_metadata_from_search(metadata, response)", assign="metadata")
                        .teardown_callback("extract_weblink(response)", assign="weblink")
                        .validate()
                        .assert_equal("status_code", 200)
                    ).when(
                        lambda vars: should_download(mode, vars.get("downloads_count")) and vars.get("is_relay") is True
                    ),
                    # Relay key path: weblink already obtained from search step
                    ConditionalStep(
                        RunRequest("download relay file")
                        .get("$weblink")
                        .teardown_callback("extract_filename_from_header(response)", assign="filename")
                        .teardown_callback("save_file(response, filename)")
                        .validate()
                        .assert_equal("status_code", 200)
                    ).when(
                        lambda vars: should_download(mode, vars.get("downloads_count"))
        ...
        return SendAnyWhereFetcher(**kwargs)
