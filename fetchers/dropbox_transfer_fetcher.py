...
class DropboxTransferFetcherFactory:
    ...
    def is_relevant_url(cls, url: str) -> bool:
        ...
        is_dropbox_host = host == "dropbox.com" or host.endswith(".dropbox.com")
        return is_dropbox_host and (
        ...
        class DropboxTransferFetcher(BaseFetcher):
            ...
            def extract_metadata(self, response: ResponseObject) -> dict:
                ...
                filename = resolve_filename(
                    response.headers,
                    url_filename or "dropbox-download.bin",
                )
            ...
            fetch_steps.extend(
                [
                    ConditionalStep(
                        ...
                        .validate()
                        .assert_equal("status_code", 200)
                    ).when(lambda vars: should_download(mode, 1))
                ]
            )

            steps = info_steps if mode == Mode.INFO else fetch_steps

        return DropboxTransferFetcher(**kwargs)
