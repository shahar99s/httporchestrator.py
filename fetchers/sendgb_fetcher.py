    ...
    def create(self_factory, mode: Mode = Mode.FETCH, **kwargs) -> BaseFetcher:
        class SendgbFetcher(BaseFetcher):
            ...
            def is_direct_download(self, response: ResponseObject) -> bool:
                return any(k.lower() == "content-disposition" for k in response.headers)
            ...
            def save_if_direct(self, response: ResponseObject, direct: bool):
                if direct and should_download(mode, 1):
                    self.save_file(response, "downloaded")
                ...
                fetch_steps.extend(
                    [
                        ...
                        ConditionalStep(
                            ...
                            .get("$direct_link")
                            .headers(**self_factory.headers)
                            .teardown_callback("save_file(response, 'downloaded')")
                            .validate()
                            .assert_equal("status_code", 200)
                        ).when(
                            lambda vars: (
