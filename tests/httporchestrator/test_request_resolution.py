...
def test_build_url_joins_relative_paths():
    assert (
        build_url("https://example.com/api", "/users")
...
def test_resolve_request_supports_callables_and_request_id():
    context = build_context(
        base_url="https://example.com", state_values={"prefix": "Bearer"}
    )
    step = (
        RequestStep("resolve")
...
def test_resolve_request_keeps_literal_strings_literal():
    ...
    request_data = resolve_request_data(
        step, context, context.build_state_snapshot(step.state_values)
    )
