...
from httporchestrator.models import (ConfigData, StepResult, VariablesMapping,
                                     WorkflowIO, WorkflowSummary,
                                     WorkflowTiming)
from httporchestrator.utils import LOGGER_FORMAT, merge_variables
...
_session_variables: ContextVar[VariablesMapping] = ContextVar("session_variables", default={})
...
class HttpRunner(object):
    ...
    def _initialize(self):
        ...
        self.client = self.client or httpx.Client(verify=False)

    def set_client(self, client: httpx.Client) -> "HttpRunner":
        self.client = client
        return self
    ...
    def set_case_id(self, case_id: str) -> "HttpRunner":
    ...
    def export(self, export: List[str]) -> "HttpRunner":
    ...
    def get_export_variables(self) -> Dict:
        ...
        for var_name in export_var_names:
            if var_name not in sv:
                raise ParameterError(f"failed to export variable {var_name} from session variables {sv}")
    ...
    def __run_step(self, step):
        """run step, step maybe any kind that implements StepProtocol interface
        ...
        for i in range(step.retry_times + 1):
            try:
                ...
                if i == step.retry_times:
                    ...
                    logger.info(f"run step retry ({i + 1}/{step.retry_times} time): {step.name()} >>>>>>")
    ...
    def _run_inner(self, param: Dict = None) -> "HttpRunner":
        ...
        logger.info(f"Start to run workflow: {self.__config.name}, Workflow ID: {self.case_id}")
