...
def _require_mapping_updates(updates, hook_name: str) -> dict:
    ...
    if not isinstance(updates, Mapping):
        raise ParameterError(
            f"{hook_name} must return a mapping or None, got {type(updates).__name__}"
        )
    return dict(updates)
...
class RequestExecutor:
    ...
    def _run_assertions(
        self, request: RequestStep, response: Response, state: dict
    ) -> list[dict]:
        results = []
            ...
            try:
                ...
                if not passed:
                    raise ValidationFailure(
                        assertion.message
                        or f"assertion failed for step '{request.name}'"
                    )
                ...
                results.append(
                    {"message": assertion.message or str(exc), "result": "fail"}
                )
    ...
    def execute(self, request: RequestStep, context, engine) -> StepResult:
        ...
        step_result = StepResult(
            name=request.name, step_type=describe_step_type(request), success=False
        )
        response = None
        response_list = []
        ...
        try:
            ...
            self._workflow_logger.log_response(
                response, response_time_ms, log_details=context.flow.log_details
            )
            ...
            for after in request.after_hooks:
                updates = _require_mapping_updates(
                    after(response_object, state), "after"
                )
                if updates:
                    state.update(updates)
            ...
            step_result.data = RequestSession(
                success=True,
                req_resps=[
                    self._exchange_recorder.capture(
                        item, log_details=context.flow.log_details
                    )
            ...
            self._workflow_logger.log_state_updates(
                step_result.state_updates, context.flow.log_details
            )
            return step_result
        finally:
...
class CallFlowExecutor:
    ...
    def execute(self, step: CallFlow, context, engine) -> StepResult:
        ...
        self._workflow_logger.log_state_updates(
            result.state_updates, context.flow.log_details
        )
        return result
...
class ConditionalStepExecutor:
    ...
    def execute(self, step: ConditionalStep, context, engine) -> StepResult:
        ...
        self._workflow_logger.log_skipped_step(step.name, step.predicate)
        return StepResult(
            name=step.name,
