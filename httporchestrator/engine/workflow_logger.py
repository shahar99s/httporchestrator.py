...
from httporchestrator.models import PredicateHook
...
class WorkflowLogger:
    def start_workflow(self, context: ExecutionContext) -> int | None:
        ...
        logger.info(
            f"Start to run flow: {context.flow.name}, Flow ID: {context.case_id}"
        )
    ...
    def log_retry(
        self, step_name: str, index: int, retry_times: int, retry_interval: float
    ) -> None:
        logger.warning(
            f"run step {step_name} failed, wait {retry_interval} sec and try again"
        )
        logger.info(
            f"run step retry ({index + 1}/{retry_times} time): {step_name} >>>>>>"
        )

    def log_request(
        self, method: str, url: str, request_data: dict, log_details: bool = True
    ) -> None:
        if not log_details:
            return
    ...
    def log_response(
        self, response, response_time_ms: float, log_details: bool = True
    ) -> None:
        import httpx
        ...
        if _is_head_response(response):
            ...
            try:
                ...
                if "attachment" in content_disposition.lower():
                    body = utils.format_response_body_for_log(
                        response.content, content_type, content_disposition
                    )
                else:
                    body = utils.format_response_body_for_log(
                        response.text, content_type, content_disposition
                    )
    ...
    def log_skipped_step(self, step_name: str, predicate: PredicateHook) -> None:
        logger.warning(
            f"step '{step_name}' is skipped due to condition not met: {predicate}"
        )
