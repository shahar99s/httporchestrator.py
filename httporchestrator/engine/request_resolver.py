...
from httporchestrator.steps import RequestStep
...
def resolve_mapping(mapping: dict, state: dict, label: str = "value") -> dict:
    result = {}
        ...
        if callable(value):
            try:
                ...
                raise ParameterError(
                    f"failed to resolve {label} '{key}': {exc}"
                ) from exc
        else:
            result[key] = value
    return result
...
def resolve_request_data(
    ...
    request_data = {
        ...
        "params": resolve_mapping(request.params_values, state, "param"),
        "headers": resolve_mapping(request.header_values, state, "header"),
        "cookies": resolve_mapping(request.cookie_values, state, "cookie"),
    ...
    if context.flow.add_request_id:
        request_headers["HRUN-RequestStep-ID"] = (
            f"HRUN-{context.case_id}-{str(int(time.time() * 1000))[-6:]}"
        )
