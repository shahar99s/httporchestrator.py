...
class ExecutionContext:
    ...
    def build_state_snapshot(
        self, state_values: VariablesMapping | None = None
    ) -> VariablesMapping:
    ...
    def collect_exported_variables(self) -> VariablesMapping:
        export_names = (
            ...
            else list(self.flow.exports)
        )
        export_mapping = {}
        for name in export_names:
            if name not in self.state:
                raise ParameterError(
                    f"failed to export '{name}' from state {self.state}"
                )
    ...
    def create_summary(self) -> WorkflowSummary:
        start_at_iso = (
            datetime.utcfromtimestamp(self.start_at).isoformat()
            if self.start_at
            else ""
        )
